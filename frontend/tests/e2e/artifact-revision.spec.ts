...
test(`resource button selects server ID/version without starting a task; close removes reference (${width}px)`, async ({ teacherPage: page }) => {
  ...
  await expect(page.getByText('正在修改：《数组基础报告》 · 第 2 版')).toBeVisible();
  expect(requests).toHaveLength(0);
  const input = page.getByPlaceholder('开始输入问题…（Shift + Enter 换行）');
  await input.fill('增加两个案例');
  ...
  await expect(page.getByRole('button', { name: '取消资料引用' })).toBeEnabled();
  await page.getByRole('button', { name: '取消资料引用' }).click();
  await expect(page.getByText('正在修改：《数组基础报告》 · 第 2 版')).toHaveCount(0);
