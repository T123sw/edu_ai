...
@lru_cache(maxsize=8)
def _build_standard_resource_repository(database_url: str):
    if not database_url:
        raise DatabaseNotConfigured("DATABASE_URL is not configured")
    return StandardResourceRepository(create_engine(database_url, pool_pre_ping=True))


def get_standard_resource_repository():
    database_url = str(os.getenv("DATABASE_URL", "")).strip()
    return _build_standard_resource_repository(database_url)
...
@lru_cache(maxsize=8)
def _build_app_state_repository(database_url: str):
    if not database_url:
        raise DatabaseNotConfigured("DATABASE_URL is not configured")
    return PostgresAppStateRepository(
        create_engine(database_url, pool_pre_ping=True)
    )


def get_postgres_app_state_repository():
    database_url = str(os.getenv("DATABASE_URL", "")).strip()
    return _build_app_state_repository(database_url)


@lru_cache(maxsize=8)
def _build_learning_repository(database_url: str):
    if not database_url:
        raise DatabaseNotConfigured("DATABASE_URL is not configured")
    return PostgresLearningRepository(create_engine(database_url, pool_pre_ping=True))
...
def get_resource_learning_repository():
    database_url = str(os.getenv("DATABASE_URL", "")).strip()
    return _build_resource_learning_repository(database_url)
