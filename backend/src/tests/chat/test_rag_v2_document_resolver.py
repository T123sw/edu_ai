...
def test_load_rag_document_content_resolves_identifier_and_loads_text(monkeypatch):
    temp_file = Path(__file__).resolve().parents[4] / "_runtime_import_test_tmp_root" / "resolver_content.md"
...
def test_load_rag_document_content_sorts_pdf_pages(monkeypatch):
    temp_file = Path(__file__).resolve().parents[4] / "_runtime_import_test_tmp_root" / "resolver_content.pdf"
    temp_file.parent.mkdir(parents=True, exist_ok=True)
