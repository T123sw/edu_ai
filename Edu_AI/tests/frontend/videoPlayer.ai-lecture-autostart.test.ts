...
assert.match(page, /AI_LECTURE_AUTOSTART_EVENT/, 'VideoPlayer should share the same-tab autoplay handoff event name');
...
assert.match(page, /window\.addEventListener\(AI_LECTURE_AUTOSTART_EVENT,\s*handleAutoStartEvent as EventListener\)/, 'VideoPlayer should listen for same-tab autoplay handoff events');
assert.match(page, /setAutoStartRequest\(event\.detail\)/, 'VideoPlayer should consume the same-tab handoff event payload without requiring a remount');
