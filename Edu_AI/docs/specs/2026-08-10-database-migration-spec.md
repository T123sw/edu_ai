...
- 观察期内，将该域模式从 `postgres_dual` 切回 `json`。
- 以迁移期间保留的 JSON 兼容投影为回滚源。
- PostgreSQL 数据不得删除；记录回滚时间点并生成增量差异报告。
...
下一阶段从 Phase 1 开始：先定义 `UserRepository`、`CourseRepository` 和 `CourseMembershipRepository` 契约，用同一组测试约束 JSON 与 PostgreSQL 实现；随后接入影子双写和差异报告。此阶段不切换读取源，也不删除任何 JSON。
