...
- [x] RED：断言八个正式资源类型和固定顺序。
- [x] RED：断言 AI 课堂主卡在资源网格之前，网格不固定为两行。
- [x] RED：断言容器允许 `min-width: 0`，卡片网格使用自适应列，页面无横向溢出。
- [x] GREEN：抽取注册表并恢复 PPT、闪卡卡片。
- [x] GREEN：实现 3/2/1 列容器响应式、单一纵向滚动和键盘操作。
- [x] GREEN：更新空状态，去除“只有六个功能”的硬编码。
- [x] 验证布局边界单元测试、1280px 实际浏览器无横向溢出、生产构建和八入口可访问性。
- [x] 提交并 push。
