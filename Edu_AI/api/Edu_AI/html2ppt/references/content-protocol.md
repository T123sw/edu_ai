...
- `Blocks` 的内容形态
- 是否包含媒体
- `deck_design_plan.md` 的页面规划
- `layout-catalog.json` 与 `component-catalog.json` 的注册信息
- 相邻页避免重复的策略

自动决定最终版式。

补充说明：
- 版式选择后的教学表达由 `deck_design_plan.md` 继续细化，包含 Teaching Objective、Teaching Recipe、Density 与 Fallback。
- 如果源内容没有可靠数字或外部事实，PPT 模块应优先使用概念图、矩阵、对比或流程，而不是编造统计信息。
...
- 尽量避免相邻页重复使用同一种文本版式
- 若存在媒体，优先选择媒体版式
- 具体选择细节以 `deck_design_plan.md`、`layout-catalog.json` 和 `component-catalog.json` 为准
