...
class VectorStore:
    ...
    def __init__(
        ...
        # BM25索引（用于关键词检索，延迟初始化）
        self._bm25_index: Optional[Any] = None
        self._bm25_documents: List[Dict] = []  # 存储文档内容用于BM25检索
        ...
        if allowed_sources and len(allowed_sources) > 0:
            ...
            if len(unique_sources) == 1:
                ...
                try:
                    ...
                    print(f"[VectorSearch] 尝试使用 $in 操作符: {where_condition}")
                    results = self.collection.query(
                        query_embeddings=[query_embedding],
                        n_results=min(top_k * 5, 100),
                        where=where_condition
        ...
        try:
            ...
            for page_num in range(len(pdf_document)):
                ...
                for img_info in image_list:
                    ...
                    if not image_bytes or width < 100 or height < 100:
                        continue
...
class RAGSystem:
    ...
    def query(
        ...
        use_rag: bool = True,  # 新增RAG开关参数
        selected_doc_ids: Optional[List[str]] = None,  # 用户选中的文档ID列表（file_path）
        ...
        # 1. 系统提示词
        if use_rag:
            # RAG模式：教育知识助手（检索结果作为约束和参考，允许模型补充）
            messages.append({
                "role": "system",
                "content": """你是一名专业的教育知识助手。在回答问题时，请遵循以下原则：

1. **优先使用检索到的参考资料**：如果提供了【参考资料】，请优先基于这些资料回答，确保答案的准确性和相关性。

2. **灵活补充和扩展**：检索结果只是起点，你可以：
   - 基于检索内容进行深入解释和扩展
   - 补充相关的背景知识、实例和案例
   - 提供更全面的分析和见解
   - 结合你的专业知识，使回答更加完整和实用

3. **保持回答的自然性**：不要让回答显得生硬或完全照搬检索内容，而是要将检索信息自然地融入到你的回答中。

4. **信息不足时的处理**：如果检索结果不足以回答完整问题，你可以：
   - 基于检索内容给出部分答案
   - 结合你的专业知识进行补充
   - 明确说明哪些部分来自检索资料，哪些是你的补充

请确保回答既准确（基于检索资料），又丰富（补充相关知识），同时保持自然流畅的表达。"""
            })
        else:
            # 自由对话模式：通用助手
            messages.append({
        ...
        if use_rag:
            ...
            if selected_doc_ids and len(selected_doc_ids) > 0:
                ...
                if selected_index_keys:
                    ...
                    print(f"[RAG] allowed_sources_for_search: {allowed_sources_for_search}")
                else:
            ...
            retrieved_docs = self.vector_store.hybrid_search(
                ...
                keyword_weight=0.4,  # 关键词检索占比40%
                vector_weight=0.6,    # 向量检索占比60%
            ...
            for doc in retrieved_docs:
                ...
                # 调试：打印检索到的文档的source
                print(f"[RAG] 检查文档: source={doc_source}")
                ...
                if matched:
                    ...
                    if matched_key in self.document_index:
                        ...
                        if (meta.get("include_in_search", True) and 
                            ...
                            print(f"[RAG] 文档通过过滤: {doc_source} -> {matched_key}")
                        else:
                            print(f"[RAG] 文档被过滤: {doc_source} include_in_search={meta.get('include_in_search')} owner={meta.get('owner')}")
                    else:
                        # 文档不在 document_index 中，说明已被删除，不应该参与检索
                        print(f"[RAG] 警告：文档 {doc_source} 不在 document_index 中（可能已被删除），已过滤")
                else:
                    print(f"[RAG] 文档被过滤: {doc_source} 不在 allowed_sources 中（allowed_sources: {list(allowed_sources)[:3]}...）")
...
---
请基于以上参考资料回答用户问题，可以适当补充相关知识使回答更完整。"""
            else:
                if not filtered_docs:
    ...
    def _call_llm(
        ...
        print(
            "[rag_llm_debug] "
            f"model={model_name} base={api_base} "
    ...
    def get_document_details(self, file_path: str, sample_limit: int = 5) -> Dict:
        ...
        # 前端/调用方传入的 file_path 可能是索引 key（含 user 前缀），展示时优先返回 physical_path
        display_path = record.get("physical_path") or file_path
