...
import { createVideoRenderItem } from './media-processor.js';
...
async function processSlide(root, slide, pptx, globalOptions = {}) {
  ...
  const finalQueue = renderQueue.filter(
    (item) =>
      !item.skip &&
      (item.type !== 'image' || item.options.data) &&
      (item.type !== 'media' || item.options.data)
  );
  ...
  for (const item of finalQueue) {
    ...
    if (item.type === 'media') slide.addMedia(item.options);
...
function prepareRenderItem(
  ...
  // --- ASYNC JOB: VIDEO Tags ---
  if (node.tagName === 'VIDEO') {
    return createVideoRenderItem({
      node,
      style,
      x,
      y,
      ...
      zIndex,
      domOrder,
      captureElementImage: elementToCanvasImage,
    });
  }
