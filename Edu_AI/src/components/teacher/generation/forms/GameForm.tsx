...
export function GameForm({ value, onChange, errors = {} }: GenerationFormProps<GameConfig>) {
  ...
  return <div className="generation-factory__form" data-resource-form="game">
    ...
    <GenerationField label="难度"><select value={value.difficulty} onChange={(event) => patch({ difficulty: event.target.value as GameConfig["difficulty"] })}><option value="easy">基础</option><option value="medium">中等</option><option value="hard">挑战</option></select></GenerationField>
    <GenerationField label="课堂用时（分钟）" required error={errors.durationMinutes}><input type="number" min={1} max={60} value={value.durationMinutes} onChange={(event) => patch({ durationMinutes: Number(event.target.value) })} /></GenerationField>
