...
export function MindMapForm({ value, onChange, errors = {} }: GenerationFormProps<MindMapConfig>) {
  ...
  return <div className="generation-factory__form" data-resource-form="mind_map">
    ...
    <GenerationField label="关系说明"><textarea value={value.description} onChange={(event) => patch({ description: event.target.value })} /></GenerationField>
    <GenerationField label="层级深度" required error={errors.depth}><input type="number" min={2} max={5} value={value.depth} onChange={(event) => patch({ depth: Number(event.target.value) })} /></GenerationField>
