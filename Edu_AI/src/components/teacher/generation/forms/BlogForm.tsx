...
export function BlogForm({ value, onChange, errors = {} }: GenerationFormProps<BlogConfig>) {
  ...
  return <div className="generation-factory__form" data-resource-form="blog">
    ...
    <GenerationField label="目标读者" required error={errors.audience}><input value={value.audience} onChange={(event) => patch({ audience: event.target.value })} /></GenerationField>
    ...
    <GenerationField label="文章结构"><input value={value.structure} onChange={(event) => patch({ structure: event.target.value })} /></GenerationField>
