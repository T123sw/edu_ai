...
export function QuizForm({ value, onChange, errors = {} }: GenerationFormProps<QuizConfig>) {
  ...
  return <div className="generation-factory__form" data-resource-form="quiz">
    ...
    <details><summary>更多设置</summary><GenerationField label="适用对象"><input value={value.audience} onChange={(event) => patch({ audience: event.target.value })} /></GenerationField><div className="generation-choice-grid"><label><input type="checkbox" checked={value.includeAnswers} onChange={(event) => patch({ includeAnswers: event.target.checked })} />附带答案</label><label><input type="checkbox" checked={value.includeExplanations} onChange={(event) => patch({ includeExplanations: event.target.checked })} />附带解析</label></div></details>
  </div>;
}
