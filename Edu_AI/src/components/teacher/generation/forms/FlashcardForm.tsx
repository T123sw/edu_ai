...
export function FlashcardForm({ value, onChange, errors = {} }: GenerationFormProps<FlashcardConfig>) {
  ...
  return <div className="generation-factory__form" data-resource-form="flashcard">
    ...
    <GenerationField label="分类"><input value={value.category} onChange={(event) => patch({ category: event.target.value })} /></GenerationField>
    <label className="generation-inline-check"><input type="checkbox" checked={value.showSource} onChange={(event) => patch({ showSource: event.target.checked })} />卡片中显示资料来源</label>
  </div>;
}
