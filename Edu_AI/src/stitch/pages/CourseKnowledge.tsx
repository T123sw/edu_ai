...
function currentView(): CourseKnowledgeView {
  return readTeacherCourseLocation(window.location.hash).view ?? "documents";
}
  ...
  return (
    <div className="course-knowledge">
      <nav className="course-knowledge__tabs" aria-label="课程知识视图">
        <a href={buildTeacherCourseHash("knowledge", courseId, { view: "documents" })} aria-current={view === "documents" ? "page" : undefined}>课程资料</a>
        <a href={buildTeacherCourseHash("knowledge", courseId, { view: "structure" })} aria-current={view === "structure" ? "page" : undefined}>知识结构</a>
      </nav>
