    ...
    <AppSurface className="flex min-h-screen">
      <SidebarDock className="h-screen gap-3 bg-[linear-gradient(180deg,#fcfdff_0%,#f2f6ff_100%)] p-4">
        <div className="mb-2 px-2 py-4">
          ...
          <p className="mt-1 text-sm text-(--muted-text)">详情编辑</p>
        </div>
      ...
      <main className="flex flex-1 flex-col">
        <header className="sticky top-0 z-40 border-b border-(--shell-border) bg-(--app-bg)/88 px-6 py-4 backdrop-blur-xl sm:px-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="mt-2 text-3xl font-black tracking-tight text-(--accent-strong) sm:text-4xl">{course.title}</h1>
            </div>
          ...
          <div className="w-full">
            <GlassPanel className="border border-(--shell-border) bg-white/90 p-6 sm:p-7">
              <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-(--accent-strong)">基本信息</p>
                  <h2 className="mt-2 text-2xl font-black text-(--accent-strong)">课程内容编辑</h2>
                </div>
              ...
              <div className="space-y-5">
                <label className="block">
                  <span className="mb-2 block text-sm font-semibold text-(--app-text)">课程名称</span>
                  <input
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-semibold text-(--app-text)">课程简介</span>
                  <textarea
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-semibold text-(--app-text)">教学目标</span>
                  <textarea
                  ...
                  />
                  <p className="mt-2 text-xs text-(--muted-text)">每行一个教学目标，保存时会自动转换为数组。</p>
                </label>

                <div className="block">
                  <span className="mb-2 block text-sm font-semibold text-(--app-text)">课程图片</span>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
                  ...
                  <div className="overflow-hidden rounded-[24px] border border-(--shell-border) bg-(--surface-subtle)">
                    <div className="aspect-16/7 bg-slate-100">
                      {formState.coverImage ? (
                        <img alt="课程封面预览" className="h-full w-full object-cover" src={formState.coverImage} />
                      ) : (
                        <div className="grid h-full place-items-center text-sm text-(--muted-text)">暂未选择课程图片</div>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-3 p-4">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full bg-(--accent) px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"
                      >
                  ...
                  <p className="mt-2 text-xs text-(--muted-text)">支持选择本地照片，保存后会直接用于课程列表和详情页展示。</p>
