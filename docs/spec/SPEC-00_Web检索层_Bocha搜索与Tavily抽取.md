...
> 最近更新：2026-07-01 · 状态：草案 v0.1（待实现）
...
- 不立即物理删旧代码（EduAgent deepsearch / crawler / SearxNG / automation_spider）→ **先影子后下线，Phase 6 删**。
...
| `siteName` | 站名 | `site` |
| `data.images` | 图片链接 | 暂存 `images`（媒体阶段用，Phase 5） |
...
- 旧 EduAgent 依赖（`deepsearch_loader` / `crawler_service`）**保留但不再默认调用**；`WEB_SEARCH_PROVIDER` 决定走新链路。
...
## 11. 旧链路下线登记（Phase 6，先影子后下线）

新链路验收通过后，Phase 6 物理删（删前 grep 依赖）：
`EduAgent/deepsearch.py`、`EduAgent/services/crawler_service.py`、`EduAgent/automation_spider`（或 `edu_ai/automation_spider`）、SearxNG（docker）、`app/deepsearch_pipeline.py`、`app/deepsearch_loader.py` 及 `deepsearch_service` 里的 EduAgent 分支。**保留** `deepsearch_importer` / storage / 端点。
