...
Merge the new frontend source snapshot from remote branch `origin/New_frontend` into the current project frontend path `Edu_AI/src`.

## Source And Target

- Remote branch: `origin/New_frontend`
- Source path in remote branch: `edu_ai-main/Edu_AI/src`
- Target path in this repository: `Edu_AI/src`

The remote branch does not contain `Edu_AI/src` at the repository root. The source path must be remapped from `edu_ai-main/Edu_AI/src` to `Edu_AI/src`.
...
1. Copy the remote tree `origin/New_frontend:edu_ai-main/Edu_AI/src`.
2. Place it at `Edu_AI/src`.
...
- `Edu_AI/src` in the isolated branch matches the remote new frontend source after path remapping.
- The `stitch/` frontend module from the remote branch is present under `Edu_AI/src/stitch`.
