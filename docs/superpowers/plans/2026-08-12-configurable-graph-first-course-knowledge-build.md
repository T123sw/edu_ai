...
> **日期**：2026-08-12
> **状态**：已实施并通过核心真实 E2E；扩展运维场景按 ACC-14 继续签收
...
| D14-020 | 2026-08-12 | Bocha 搜索异常或返回空结果时自动降级到 Tavily Search；两者都不可用才按搜索失败进入质量门禁 | 实机验收发现现有 Bocha key 返回 403，单 provider 会让网络资料全部为 0 | 保留 Bocha 主配置兼容性，同时让真实网络材料获取不再依赖单点 provider |
| D14-021 | 2026-08-12 | 每个叶节点达到 `max(target_materials_per_leaf, minimum_web_materials_per_leaf)` 个成功网络来源后停止额外抓取，并为处理或跳过的每个候选上报 indexing 进度 | 实机抓取 10 个候选耗时过长且任务中心长时间停在 5%，超出用户配置规模的抓取没有收益 | 降低构建时延和外部调用量；后台进度与候选处理数量一致 |
| D14-022 | 2026-08-12 | 后台轮询只在本地图谱未修改时同步服务器草案；存在未保存编辑时保留教师本地草案 | 5 视口并发 E2E 暴露轮询可能在输入后覆盖编辑，使“保存草案”重新禁用 | 避免教师编辑丢失；服务器最新 revision 仍在保存/确认时执行冲突校验 |
